...
def main():
    parser = argparse.ArgumentParser(
        prog="bugzilla-readonly-mcp", description="Read-only MCP server for Bugzilla interaction."
    )
    parser.add_argument(
        "--transport",
        type=str,
        ...
        help="MCP transport mode. Use 'http' for URL-based clients and 'stdio' for command-based clients. Defaults to MCP_TRANSPORT or 'http'.",
    )
    parser.add_argument(
        "--bugzilla-server",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Bugzilla API key fallback used when request headers are unavailable (e.g., stdio mode). Defaults to BUGZILLA_API_KEY.",
    )
    ...
    if args.transport == "stdio" and args.api_key is None:
        mcp_log.critical(
            "Error: stdio transport requires --api-key or BUGZILLA_API_KEY. Exiting."
        )
        sys.exit(1)

    server.cli_args["transport"] = args.transport
    ...
    server.cli_args["api_key"] = args.api_key
