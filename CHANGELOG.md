...
### Changed
- Removed `add_comment` from the MCP tool surface to enforce a read-only server model.
- Removed the write-path client method from `src/mcp_bugzilla/mcp_utils.py` so no Bugzilla mutation endpoint is called.
- Updated tests and README examples to reflect read-only behavior.

### Breaking Changes
- The `add_comment` tool is no longer available. MCP clients must not rely on write operations.
